...
//			this.maxScrollX = this.view.offsetWidth;
			this.maxScrollX = document.body.offsetWidth ;
			this.x = this.y = 0;
		...
		go: function(pageSelector) {
			...
			if (nextPage) {
				...
				//验证
				this._trigger('pageBeforeShow', nextPage);
				if(nextPage.noGo){
					return;
				}
				var previousNavbar;
...
//单页面通用js
/**以下是个人中心页面js**/
//单页面可通用,前提是把id设置成myPage,myPageContent
//	<!--个人中心页面主结构开始-->
...
//初始化单页view
var viewApi = mui('#myPage').view({
	defaultPage: '#myPageContent'
});
...
(function($) {
	...
	view.addEventListener('pageBeforeShow', function(e) {
		...
		if (e.detail.page.id != "myPageContent") {
			Zepto(".mui-bar.mui-bar-tab").hide();
		}
	});
